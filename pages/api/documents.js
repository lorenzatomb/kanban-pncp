...
export default async function handler(req, res) {
  ...
  try {
    ...
    var prompt = "Você é um analista de licitações especializado em audiovisual, videowall, painéis LED, salas de controle, videoconferência e CFTV.\n\n" +
      ...
      '{"resumo_executivo":"string 2-3 parágrafos","is_match":true/false,"confianca":0-100,' +
      '"raciocinio":"string explicando por que é ou não relevante",' +
      '"itens":[{"nome":"string","qtd":number,"specs":"string","preco_est":number}],' +
      '"requisitos":{"certificacoes":["string"],"prazos":{"entrega":"string"},"qualificacoes":["string"],"garantias":["string"]},' +
      '"red_flags":["string"]}\n\n' +
      "LICITAÇÃO:\n" +
      "Objeto: " + opp.objeto + "\n" +
    ...
    try {
      ...
      analise = { resumo_executivo: texto, is_match: null, confianca: null, itens: [], requisitos: {}, red_flags: [] };
    }

    // Salva análise
    await supabase.from("bid_analyses").insert({
      oportunidade_id: oportunidade_id,
      status: "completed",
      resumo_executivo: analise.resumo_executivo || texto,
      itens: analise.itens || [],
      requisitos: analise.requisitos || {},
      red_flags: analise.red_flags || [],
      confirmado_match: analise.is_match,
